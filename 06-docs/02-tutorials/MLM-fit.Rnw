...
<<r plot1, eval = FALSE, warning=FALSE, message=FALSE, comment=NA>>=
set.seed(99103)
norm2 <- rnorm(1000, mean = 0, sd = 1)

gr1 <- ggplot(data=NULL,
			        aes(x = norm2)) +
		geom_histogram(aes(y = after_stat(density)),
					         alpha = 0.6) +
		stat_density(color = rgb(255, 0, 90, maxColorValue = 255),
					       linewidth = 2,
					       geom = "line") +
		theme_clean() +
		...
		labs(x = "X",
			   y = "Density",
			   main = "Histogram: normal SD")
ggsave(gr1, filename = "../../05-graphs/Supplem-01.pdf", height = 4, width = 6)
rm(gr1, norm2)
@
...
<<r plot2, eval = FALSE, warning=FALSE, message=FALSE, comment=NA>>=
set.seed(109550)
norm1 <- rnorm(1000, mean = 0, sd = 0.1)

gr2 <- ggplot(data=NULL,
              aes(x = norm1)) +
    geom_histogram(aes(y = after_stat(density)),
                   alpha = 0.6) +
    stat_density(color = rgb(255, 0, 90, maxColorValue = 255),
    ...
    labs(x = "X",
         y = "Density",
         main = "Histogram: very low SD")
ggsave(gr2, filename = "../../05-graphs/Supplem-02.pdf", height = 4, width = 6)
rm(gr2, norm1)
@
